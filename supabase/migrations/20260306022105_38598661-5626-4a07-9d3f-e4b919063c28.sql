...
BEGIN
  RETURN QUERY
  SELECT m.id, m.name, m.area, m.description, m.education, m.photo_url, m.availability, m.min_advance_hours, m.sessions_completed_count, m.linkedin_url, m.temporarily_unavailable, m.anos_experiencia
...
BEGIN
  ...
  SELECT m.id, m.name, m.area, m.description, m.education, m.photo_url, m.availability,
    m.min_advance_hours, m.sessions_completed_count, m.linkedin_url,
    COALESCE(mtd.tags, '[]'::jsonb), COALESCE(mm.match_count, 0),
    COALESCE(mm.matching_tags, '[]'::jsonb), m.temporarily_unavailable, m.anos_experiencia
