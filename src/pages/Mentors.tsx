          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6 lg:gap-8">
            {filteredMentors.map((mentor, index) => (
              <motion.div
                ...
                className={`bg-card rounded-[20px] border border-border shadow-sm overflow-hidden flex flex-col min-h-[480px] relative hover:-translate-y-2 hover:shadow-[0_20px_40px_rgba(255,107,53,0.12)] transition-all duration-300 ${
                  mentor.temporarily_unavailable ? "opacity-60 grayscale" : ""
                }`}
              >
                {/* Availability badge */}
                {!mentor.temporarily_unavailable && (
                  <div className="absolute top-3 right-3 z-10 bg-emerald-50 text-emerald-600 text-[11px] font-semibold px-2.5 py-1 rounded-lg dark:bg-emerald-950/40 dark:text-emerald-400">
                ...
                {mentor.linkedin_url && (
                  <a
                    ...
                    className="absolute top-3 left-3 z-10 text-[#0A66C2] hover:opacity-70 transition-opacity"
                ...
                <div className="p-5 flex flex-col items-center flex-1">
                  {/* Circular photo */}
                  <div
                    className="w-24 h-24 rounded-full border-[3px] border-primary overflow-hidden mb-3 shrink-0 cursor-pointer"
                  ...
                  {/* Name */}
                  <h3
                    className="text-lg font-semibold text-foreground text-center cursor-pointer hover:opacity-80 transition-opacity leading-tight"
                    onClick={() => openProfileDialog(mentor)}
                  >
                    {mentor.name}
                  </h3>
                    ...
                    <div className="flex flex-wrap justify-center gap-1.5 mt-2.5">
                      {mentor.tags.slice(0, 3).map((tag) => {
                        ...
                        return (
                          ...
                          >
                            {isMatching && <Star className="w-2.5 h-2.5 fill-amber-400 text-amber-400" />}
                            {tag.name}
                          </span>
                        );
                      })}
                  ...
                  {/* Stats grid */}
                  <div className="grid grid-cols-3 gap-1 w-full py-2.5 border-t border-border/40 text-center">
                    <div>
                      <div className="text-base font-bold text-primary leading-tight">
                        {mentor.anos_experiencia ?? "—"}
                      </div>
                      <div className="text-[10px] text-muted-foreground mt-0.5">
                        {mentor.anos_experiencia === 1 ? "ano exp." : "anos exp."}
                      </div>
                    </div>
                    <div className="border-x border-border/30">
                      <div className="text-base font-bold text-primary leading-tight">{mentor.sessions_completed_count}</div>
                      <div className="text-[10px] text-muted-foreground mt-0.5">Mentorias</div>
                    </div>
                    <div>
                      <div className="text-base font-bold text-primary leading-tight">{mentor.totalReviews}</div>
                      <div className="text-[10px] text-muted-foreground mt-0.5">Feedbacks</div>
                    </div>
                  </div>
                  ...
                  <div className="w-full pt-2">
                    {mentor.temporarily_unavailable ? (
                      <div className="w-full bg-muted text-muted-foreground py-2.5 rounded-xl font-semibold flex items-center justify-center gap-2 cursor-not-allowed text-sm">
