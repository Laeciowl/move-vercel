                  ...
                  <div className="space-y-3">
                    {upcomingSessions.map((session) => (
                      ...
                      >
                        ...
                        {session.status === "scheduled"
                          ...
                          <div className="bg-amber-50 dark:bg-amber-900/20 border border-amber-300 dark:border-amber-700/50 rounded-lg p-3 space-y-2">
                            ...
                            <p className="text-xs text-amber-600 dark:text-amber-400/80">
                              Mentores são voluntários doando tempo. Se não puder comparecer, cancele para liberar o horário.
                            </p>
                            <div className="flex gap-2">
                              <Button
                                size="sm"
                                onClick={() => handleReconfirmPresence(session.id)}
                                disabled={confirmingId === session.id}
                              ...
                              <Button
                                size="sm"
                                variant="outline"
                                onClick={() => handleCancelFromReconfirmation(session.id)}
                                disabled={confirmingId === session.id}
                                className="flex-1 h-8 text-xs gap-1.5 border-destructive/50 text-destructive hover:bg-destructive/10"
                              >
                                <XCircle className="w-3.5 h-3.5" />
                                Cancelar
                              </Button>
                        ...
                        {/* Reconfirmed badge */}
                        {session.reconfirmation_confirmed === true && (
                          <div className="flex items-center gap-1.5 px-3 py-2 bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-700/50 rounded-lg">
                            <CheckCircle className="w-4 h-4 text-green-600" />
                            <span className="text-xs font-medium text-green-700 dark:text-green-400">Presença confirmada ✅</span>
                          </div>
                        )}
